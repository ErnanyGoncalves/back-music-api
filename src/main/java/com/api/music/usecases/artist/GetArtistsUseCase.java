...
public class GetArtistsUseCase {
  ...
  public ResponseListDTO<ArtistDTO> getArtists(List<String> countries, List<String> genres, Integer page,
      Integer pageSize) {
    List<ArtistDTO> listOfArtists = artistRepository.findAll(countries, genres, page, pageSize).stream()
    ...
    return new ResponseListDTO<>(listOfArtists, new Pagination(currentPage,pageSize,totalElements,totalPages,navigationPaths));
  }
  ...
  private Navigation buildNavigation(Integer currentPage, Integer totalPages, String filters) {
    ...
    if(pPage != null) pPageStr = new StringBuilder("/artists?page="+pPage+filters).toString();
    if(nPage != null) nPageStr = new StringBuilder("/artists?page="+nPage+filters).toString();
